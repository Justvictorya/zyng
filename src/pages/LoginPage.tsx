      ...
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-8 space-y-6 shadow-xl relative z-10">
        ...
        <div className="space-y-2 mt-4">
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            <button
              onClick={() => {
                const clientId = "sbawdlq2x25w5sq8lw";
                const redirectUri = `${window.location.origin}/auth/callback?provider=tiktok`;
                const csrfState = Math.random().toString(36).substring(2);
                localStorage.setItem("tiktok_login_state", csrfState);
                const url = `https://www.tiktok.com/v2/auth/authorize/?client_key=${clientId}&scope=user.info.basic&response_type=code&redirect_uri=${encodeURIComponent(redirectUri)}&state=${csrfState}`;
